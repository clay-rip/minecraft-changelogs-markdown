...
-   Added Shelf Mushrooms
-   Added Red Shrubs
-   Added Abandoned Camp structures
-   Added new Explorer Maps
-   Added Cushions
-   Added Straw Beds
...
-   Improvements to Realms, including the ability to create and manage Invite Codes
-   Added Gothic language support
...
-   A small Shelf Mushroom can be grown into a large Shelf Mushroom by using Bone Meal
...
-   The Shelf Mushroom makes a bouncing sound when a living entity bounces on it
...
-   A new kind of structure which can be found in the following biomes:
    ...
    -   Wooded Badlands
    -   Pale Garden
-   Each of these biomes has its own variant of the structure with a design based on the biome
...
-   Cushions are crafted by using 3 Wool Slabs of the same color
...
-   Cushions cannot be moved by pistons
...
-   The Straw Bed is a new type of bed which can be used to sleep through the night without setting your spawn point
...
-   It is crafted by using 3 Hay Bales and will give you 4 Straw Beds
-   There's a new statistic in the Statistics window for the amount of times you've slept in a Straw Bed
...
![Efe is standing on a rainbow platform made of concrete stairs and slabs, with a backdrop of poplar leaves, logs and shelf mushrooms.](https://launchercontent.mojang.com/v2/images/concrete1280x720.jpg)

### Concrete Stairs and Concrete Slabs

-   Each colored Concrete block now has a new Stair and Slab variant
-   Concrete Slabs and Stairs can also be crafted using a Stone Cutter
...
-   Players can now easily join a Realm by entering an Invite Code in a new "Join Realm" screen, found in the Realms main menu
-   Added render distance and simulation distance sliders to the Realm configuration settings
...
![Trading with a villager who is holding a new explorer map.](https://launchercontent.mojang.com/v2/images/explorer1280x720.jpg)

## Changes

-   The Main Menu now has an updated background panorama showing the new Dappled Forest biome with an Abandoned Camp
-   Removed fallen leaf particles from Spruce Trees
-   Suspicious Stews can now be crafted with 2 of any mushroom
-   Mushroom Stew can now be crafted with 2 of any mushroom
-   The player icon on maps has been updated to always show the direction the player is facing
...
-   Replaced the current "Improved Transparency" approach with a new order-independent transparency algorithm which is expected to be much better at solving translucency issues
-   Added a rebindable debug key combination "F3 + X" to toggle "Improved Transparency"
-   When using a Shield together with a Hoe or a Shovel, raising the Shield will take precedence over tilling and path making
...
-   Improvements to chunk generation performance
...
-   The following Explorer Maps have been renamed to have more consistent naming
    -   Ocean Monument Explorer Map → Ocean Monument Map
...
![A slime sits in front of a geyser inside of a waterfall in a cave.](https://launchercontent.mojang.com/v2/images/oit21280x720.jpg)
...
-   Before this release, the "Improved Transparency" video setting attempted to mitigate translucency-related visual issues
...
-   Now, the "Improved Transparency" video setting will instead enable a new order-independent transparency algorithm (which we will refer to as OIT)
...
> **Developer's Note**: _Special thanks to Rubén Osorio López, Jozufozu, and the Minecraft modding community in general for sharing the information that helped us to deliver this._
...
-   Data set on a Spawn Egg will now transfer to a Mob Spawner when a player with operator permissions uses the Egg on the Spawner
-   Sherds dropped from breaking a Decorated Pot will now preserve data, for example custom names
-   Enchantments now apply to items being damaged from protecting an undead mob from the sun
-   Teleporting with an Ender Pearl now resets the momentum of the player being teleported
...
-   The debug overlay now supports a separate GUI scale from the rest of the game
...
-   The World Options button has been moved to where the Open to Lan button used to be, accessible from the pause menu
    -   The World Options screen is divided into two sections: "General" and "Multiplayer"
        -   The "General" section contains the following options:
            -   Default Game Mode: the default game mode of the world (equivalent to the `/defaultgamemode` command)
            -   Personal Game Mode: your active game mode in the current world (equivalent to the `/gamemode` command, or F3 + F4)
            -   Allow Commands: Controls whether you can use commands in this world or not
            -   Difficulty: Controls the difficulty of the world (equivalent to the `/difficulty` command)
            -   Edit Game Rules: Opens the Game Rules screen, which allows you to view and edit the game rules of the world
            -   Restrictions: View your restrictions in the world
        -   The "Multiplayer" section contains the following options:
...
-   The Online button used to access the Online Options screen will now always be shown in the Options menu instead
...
-   Improved the ordering of the Natural Blocks tab so that all inner ordering of group content sequentially progresses from Overworld → Nether → End to stay consistent with other tabs
...
-   Some log lines that server software might look out for have been removed
    ...
    -   If you are missing access to specific events, please create a feature request on the [Feedback site](https://feedback.minecraft.net/)
...
## Data Pack Versions 108.0 through 121.0

-   Commands like `/give`, `/unpublish`, and `/tick` should now properly signal errors
-   Pottery sherds are now data-driven
-   Entries stored in the `minecraft:pot_decorations` component and `minecraft:decorated_pot` block entity are now full item stacks
-   Signs no longer automatically execute click events in custom text
-   Potion recipes are now data-driven
...
-   The codec for element lists/tags now allows mixing inline values and references
...
![Looking at several villagers while having the spider vision effect enabled.](https://launchercontent.mojang.com/v2/images/posteffects1280x720.jpg)
...
-   Evaluates `minecraft:context_int_provider` and `minecraft:context_float_provider` values in various contexts
-   In all contexts, `origin` is set to the current command context position, while `this` entity is set to `@s`
-   If the evaluation fails (for example due to division by zero), this command will fail

Syntax:

-   `/compute <target> integer <int_provider>` OR
-   `/compute <target> float <float_provider> [<scale>]`, where:
    -   `<target>` - one of:
        -   `default` - runs the provider in the `minecraft:command_compute_default` context, where only common arguments (position and `this` entity) are available
        -   `block <pos>` - runs the provider in the `minecraft:command_compute_position` context, where block state and block position are set based on the block at position `<pos>`
        -   `entity <entity selector>` - runs the provider in the `minecraft:command_compute_entity` context, where `target_entity` is set to the selector result
    -   `<int_provider>` - either an element of the `minecraft:context_int_provider` registry or an inline value
    -   `<float_provider>` - either an element of the `minecraft:context_float_provider` registry or an inline value
    -   `<scale>` - optional scale value, defaults to `1.0`
        -   The final result will be multiplied by `<scale>` and rounded towards negative infinity

#### Changes to `data`

-   Added new source to `/data modify ... append|insert|merge|prepend|set` called `compute`
-   It evaluates a `minecraft:context_int_provider` or `minecraft:context_float_provider` value and then provides the result either as an integer or a float tag
-   Example: `/data modify storage foo bar set compute entity @n[type=minecraft:armor_stand] integer {type:score,target:{type:context,target:target_entity},score:baz}`

Syntax:

-   `compute <target> (integer <int_provider> | float <float_provider>)`, where the syntax of `target`, `int_provider`, `float_provider` is the same as in the `/compute` function
    -   Note: unlike `/compute`, there is no `scale`, since the result is directly stored as a float, instead of being returned from the command as an integer

#### Changes to `give`

-   When the amount of items passed to the `give` command is above the limit, the command will now signal an error instead of just printing an error message and returning `0`
...
-   The result of the `team join` and `team leave` subcommands now reflects the exact count of entities that have joined or left (i.e. excluding entities for which teams have not changed)

#### Changes to `item`

-   Now takes a slot source as an argument in place of anywhere a slot was previously used
    ...
    -   The `replace` sub-command has new semantics:
        -   The contents of each destination slot are replaced with the corresponding source item one by one in the order they are provided by the slot source
...
-   The `feature` sub-command now allows inline definition of the Feature
...
-   Added a new optional argument to set the swing animation to play
    -   Allowed values: `none`, `whack`, `stab`
...
![Zuri is creating a trail of shroomlights using a blaze rod as a custom block transformer.](https://launchercontent.mojang.com/v2/images/blocktransform1280x720.jpg)
...
-   Format:
    -   namespaced ID pointing to an element of the `minecraft:decorated_pot_pattern` registry

#### Added `minecraft:cooking_fuel`

-   Describes an item that can be used as fuel for a Furnace, Smoker, or Blast Furnace
-   Format: object with fields
    -   `burn_time` - inline integer number, or a namespaced ID pointing to an element of the `minecraft:context_int_provider` registry representing the time in ticks for which this fuel will burn
    -   `speed_multiplier` - inline float number, or a namespaced ID pointing to an element of the `minecraft:context_float_provider` registry representing the speed of the cooking/smelting
...
-   Format: object with fields
    -   `uses` - inline integer number, or a namespaced ID pointing to an element of the `minecraft:context_int_provider` registry representing the number of times this fuel will brew before being consumed
    -   `speed_multiplier` - inline float number, or a namespaced ID pointing to an element of the `minecraft:context_float_provider` registry representing the speed of the brewing
...
#### Changed `minecraft:pot_decorations`
...
#### Changed `minecraft:trim_material`

-   The format of inline material definitions has been updated as described below

#### Added `minecraft:block_transformer`

-   Allows an item to transform blocks into other blocks on player interaction
-   Format:
    -   a namespaced ID of a `minecraft:block_transformer` registry type

#### Added `minecraft:sign_text_front` and `minecraft:sign_text_back`

-   Two identical components that represent text on the front and back of a sign
-   Contents will be displayed in the item's tooltip
-   The format is the same as the `front_text` and `back_text` fields on the `minecraft:sign` and `minecraft:hanging_sign` block entities, i.e.
    ...
    -   `filtered_messages` - an optional list of text components, if omitted, it will be set to the same value as `messages`
        -   `messages` and `filtered_messages` must have the same entry count
...
-   Format: object with fields
    -   `layers` - inline integer number, or namespaced ID pointing to an element of the `minecraft:context_int_provider` registry
...
-   Format: object with fields
    -   `targeting_entity_types` - an entity ID, a list of namespaced entity IDs, or a hash-prefixed entity tag to match
...
-   Format: object with fields:
    -   `type` - swing animation type, one of `none`, `whack`, `stab`
    -   `duration` - non-negative integer, the length of the animation in ticks
...
-   `override_armor_assets` has been removed
    -   Overrides are now defined in the Resource Pack Equipment Asset
...
-   Added `invulnerable_time` tag which makes an entity temporarily invulnerable for a specific number of ticks

![Noor is sitting on a cushion waiting for smelting to finish, while snacking on a carrot.](https://launchercontent.mojang.com/v2/images/furnace1280x720.jpg)

### Block Entity Data

#### Changed `minecraft:decorated_pot`

The `sherds` field no longer stores a list of 4 item IDs. Instead, the new format is an object with 4 optional item stack fields named:
...
-   By default, commands and other click events contained in Sign text are no longer executed when the block is clicked
    ...
    -   A new boolean field called `allow_op_features` (defaults to `false`) has been added to restore previous behavior
...
-   `total_brew_time` - integer, the total amount of time the current brewing process will take
-   `total_fuel` - integer, represents the amount of fuel that was added in the last refuel
-   `speed_multiplier` - float, used to speed up or slow down the next brewing process
...
-   Visible root advancements (i.e. ones that have a `display` field, but no `parent` field) must always declare a `background` field in the `display` structure
-   Only root advancements can have a `background` field in the `display` structure
...
-   Multiple fields that accepted a list of loot conditions are now stricter:
    -   The previous functionality of a list of conditions being equivalent to a single `minecraft:all_of` condition has been removed
    -   Additionally, the `type` field is now mandatory (previously it defaulted to `minecraft:entity_properties`)
...
##### Changed `minecraft:any_block_use`
...
##### Changed `minecraft:bred_animals`

-   The `child`, `parent`, and `partner` fields now accept an inline value or a namespaced ID of a `minecraft:predicate` type

##### Changed `minecraft:brewed_potion`

-   The `potion` field now accepts a `minecraft:potion_contents` data component predicate

##### Changed `minecraft:cured_zombie_villager`

-   The `villager` and `zombie` fields now accept an inline value or a namespaced ID of a `minecraft:predicate` type

##### Changed `minecraft:default_block_use`

-   The `location` field now accepts an inline value or a namespaced ID of a `minecraft:predicate` type

##### Changed `minecraft:effects_changed`
...
-   The `bystander` and `lightning` fields now accept an inline value or a namespaced ID of a `minecraft:predicate` type
...
-   The `loot_table` field has been renamed to `loot_tables` and now accepts a single namespaced ID, a list of namespaced IDs, a list of inline values, or a hash-prefixed tag ID of a `minecraft:loot_table` type
    -   If a tag or a list is used, the trigger will run when any listed loot table is generated
...
-   The `recipe_id` field has been renamed to `recipes` and now accepts a single namespaced ID, a list of namespaced IDs, or a hash-prefixed tag ID of a `minecraft:recipe` type
    -   If a tag or a list is used, the trigger will run when any listed recipe is used
...
-   The `recipe` field has been renamed to `recipes` and now accepts a single namespaced ID, a list of namespaced IDs, or a hash-prefixed tag ID of a `minecraft:recipe` type
    -   If a tag or a list is used, the trigger will run when any listed recipe is unlocked
...
![Noor is sitting on a cushion waiting for some potions to brew.](https://launchercontent.mojang.com/v2/images/brewing1280x720.jpg)
...
![Noor is using a spyglass to get a closer look at some large mesa formations in the distance.](https://launchercontent.mojang.com/v2/images/worldgen1280x720.jpg)
...
![A long view of a dappled forest at sunset, with a patch of cherry trees in the distance.](https://launchercontent.mojang.com/v2/images/dappledforest1280x720.jpg)
...
> **Developer's Note**: _Vertex inputs in previous versions using OpenGL were not allowed to specify a location. Compatibility can be preserved by only specifying locations when the "VULKAN" macro is defined._

> **Developer's Note**: _Vertex outputs and fragment inputs in previous versions were matched based on name alone. Compatibility can be preserved by continuing to use identical names._
...
![Makena is enjoying a rest on a cushion while orange poplar leaves rain down from above.](https://launchercontent.mojang.com/v2/images/particles1280x720.jpg)
...
-   [MC-40597](https://bugs.mojang.com/browse/MC-40597) - The interaction of the game's built-in fullscreen mode and macOS fullscreen can glitch the game window
...
-   [MC-73162](https://bugs.mojang.com/browse/MC-73162) - Breaking an entity that instantly breaks sometimes destroys the block behind it in Creative mode
-   [MC-90148](https://bugs.mojang.com/browse/MC-90148) - When a player gets out of a boat, the other passenger's head remains rotated sideways
-   [MC-91006](https://bugs.mojang.com/browse/MC-91006) - Some entities lose their name when placed or broken
-   [MC-96365](https://bugs.mojang.com/browse/MC-96365) - Boats and rafts retain their rotational momentum after exiting and entering them
-   [MC-102732](https://bugs.mojang.com/browse/MC-102732) - Arrows can visually bounce off of entities when barely missing them
-   [MC-102836](https://bugs.mojang.com/browse/MC-102836) - Endermen can teleport to some dangerous places
...
-   [MC-110989](https://bugs.mojang.com/browse/MC-110989) - `/spreadplayers` considers signs to be valid blocks to be spread onto
...
-   [MC-113006](https://bugs.mojang.com/browse/MC-113006) - Items reeled in with fishing rods do not move smoothly but appear to teleport
-   [MC-116286](https://bugs.mojang.com/browse/MC-116286) - `/spreadplayers` can spread entities on structure void
-   [MC-121278](https://bugs.mojang.com/browse/MC-121278) - Key combinations are mapped to the US keyboard layout even if using another layout
-   [MC-121367](https://bugs.mojang.com/browse/MC-121367) - The game doesn't use borderless fullscreen
-   [MC-121375](https://bugs.mojang.com/browse/MC-121375) - The Command (⌘) key on macOS is displayed as "Win" in the Key Binds screen
-   [MC-122477](https://bugs.mojang.com/browse/MC-122477) - Opening the chat sometimes appends the character the used key types to the message on Linux
-   [MC-122717](https://bugs.mojang.com/browse/MC-122717) - Success messages for some commands include unaffected entities, unlike the command result
-   [MC-122731](https://bugs.mojang.com/browse/MC-122731) - Camera movement stutters when turning in a boat
-   [MC-127939](https://bugs.mojang.com/browse/MC-127939) - Several keys are displayed as "scancode.number" in the Key Binds menu
-   [MC-133407](https://bugs.mojang.com/browse/MC-133407) - Key binds switch when upgrading from before 1.13 if not using a QWERTY keyboard
...
-   [MC-149058](https://bugs.mojang.com/browse/MC-149058) - Using hotkeys to move items into the anvil UI appends the character the used key types to the new item name
-   [MC-151857](https://bugs.mojang.com/browse/MC-151857) - The success message of `/give` when affecting multiple players uses the wrong string
...
-   [MC-159283](https://bugs.mojang.com/browse/MC-159283) - Terrain generation fails in progressively shrinking rings centered around the End's center
-   [MC-161885](https://bugs.mojang.com/browse/MC-161885) - Many transparent things do not render behind slimes
-   [MC-161898](https://bugs.mojang.com/browse/MC-161898) - Transparent particles make other particles behind them invisible
...
-   [MC-162159](https://bugs.mojang.com/browse/MC-162159) - Riding boats above bubble columns causes players to teleport up 2 blocks
-   [MC-168446](https://bugs.mojang.com/browse/MC-168446) - Single dark oak and pale oak saplings accept bone meal despite not being able to grow
...
-   [MC-174958](https://bugs.mojang.com/browse/MC-174958) - `/gamerule` succeeds even when the value is not changed
-   [MC-175180](https://bugs.mojang.com/browse/MC-175180) - The `Silent` NBT tag still doesn't make some entities fully silent
-   [MC-183446](https://bugs.mojang.com/browse/MC-183446) - Endermen still avoid projectiles when riding minecarts or boats
-   [MC-187669](https://bugs.mojang.com/browse/MC-187669) - The world border is too dark with the "Improved Transparency" option enabled
...
-   [MC-206356](https://bugs.mojang.com/browse/MC-206356) - The motion of falling blocks is visually slow
-   [MC-206540](https://bugs.mojang.com/browse/MC-206540) - Increased input delay when riding an entity
-   [MC-210117](https://bugs.mojang.com/browse/MC-210117) - Sculk sensors don't detect ice and snow melting
-   [MC-210584](https://bugs.mojang.com/browse/MC-210584) - Water does not render behind items modified to use semi-transparent textures
-   [MC-210734](https://bugs.mojang.com/browse/MC-210734) - Translucent blocks and other entities render over invisible spiders' eyes regardless of positions
-   [MC-212671](https://bugs.mojang.com/browse/MC-212671) - Loot table entries of type `tag` with `expand` set to true do not apply loot functions to the items
-   [MC-214113](https://bugs.mojang.com/browse/MC-214113) - Endermen and shulkers can teleport onto the Nether roof
-   [MC-218015](https://bugs.mojang.com/browse/MC-218015) - Translucent blocks and other entities render over invisible endermen's eyes regardless of positions
-   [MC-218156](https://bugs.mojang.com/browse/MC-218156) - Shipwrecks and ocean ruins can generate with empty buried treasure maps
-   [MC-222559](https://bugs.mojang.com/browse/MC-222559) - Block hitboxes and entity hitboxes don't render behind semi-transparent entities
-   [MC-222818](https://bugs.mojang.com/browse/MC-222818) - You can ignite TNT in Adventure mode
-   [MC-223321](https://bugs.mojang.com/browse/MC-223321) - You can destroy turtle eggs by standing or jumping on them in spawn protection
-   [MC-223323](https://bugs.mojang.com/browse/MC-223323) - You can trample farmland in spawn protection
...
-   [MC-233888](https://bugs.mojang.com/browse/MC-233888) - Pressing the Space or Enter keys to interact with elements results in the pressed key being executed again
...
-   [MC-248666](https://bugs.mojang.com/browse/MC-248666) - Particles produced from dragon eggs teleporting cannot be seen by other players
...
-   [MC-252934](https://bugs.mojang.com/browse/MC-252934) - Placing structures with paintings, item frames, or glow item frames logs `Block-attached entity at invalid position: [BlockPosition]`
-   [MC-253023](https://bugs.mojang.com/browse/MC-253023) - Breaking paintings, item frames, or glow item frames cannot trigger sculk shriekers
...
-   [MC-258579](https://bugs.mojang.com/browse/MC-258579) - The player's head does not rotate with a delay from the camera in third person view while riding a vehicle
...
-   [MC-263708](https://bugs.mojang.com/browse/MC-263708) - The Key Binds screen renders keypad keys the same as regular keys
-   [MC-264003](https://bugs.mojang.com/browse/MC-264003) - Boats carrying players sometimes stop abruptly when sliding along ice
...
-   [MC-265584](https://bugs.mojang.com/browse/MC-265584) - TNT dispensed in bubble columns with soul sand right under visually moves through blocks above
...
-   [MC-266231](https://bugs.mojang.com/browse/MC-266231) - Mouse buttons above 8 can't be set as key binds
-   [MC-266429](https://bugs.mojang.com/browse/MC-266429) - Holding Shift on macOS causes the hotbar scroll direction to reverse when scrolling with the mouse
...
-   [MC-268155](https://bugs.mojang.com/browse/MC-268155) - Rendering gaps in the world under specific conditions
-   [MC-268521](https://bugs.mojang.com/browse/MC-268521) - The game window doesn't have a minimal width and height
-   [MC-269968](https://bugs.mojang.com/browse/MC-269968) - Can't scroll with the mouse while holding Shift on macOS (except in the hotbar)
-   [MC-270706](https://bugs.mojang.com/browse/MC-270706) - Trying to brew an uncraftable potion into a splash or lingering potion consumes the reagent but keeps the potion exactly as is
...
-   [MC-271723](https://bugs.mojang.com/browse/MC-271723) - Translucent blocks and other entities render over invisible phantoms' eyes regardless of positions
-   [MC-271724](https://bugs.mojang.com/browse/MC-271724) - Translucent blocks and other entities render over invisible breezes' eyes regardless of positions
-   [MC-272801](https://bugs.mojang.com/browse/MC-272801) - Particles and translucent objects do not render when behind the ender dragon's death animation
...
-   [MC-274924](https://bugs.mojang.com/browse/MC-274924) - Angry endermen suffocating while in boats or minecarts can attack their target from really far away
...
-   [MC-276692](https://bugs.mojang.com/browse/MC-276692) - Lingering potions which land on the target's head do not grant their effects to the target
-   [MC-277772](https://bugs.mojang.com/browse/MC-277772) - Stained glass renders incorrectly through item models in stained glass with the "Improved Transparency" option enabled
-   [MC-277906](https://bugs.mojang.com/browse/MC-277906) - Ender pearls land incorrectly at high speeds
-   [MC-278336](https://bugs.mojang.com/browse/MC-278336) - The player model still flickers at very high speeds
-   [MC-278557](https://bugs.mojang.com/browse/MC-278557) - Exiting a spinning boat and then inserting a mob makes its head spin forever
...
-   [MC-279285](https://bugs.mojang.com/browse/MC-279285) - Experience orbs that have been stuck inside blocks rapidly accelerate toward the ground if they've been disturbed by a player's presence
-   [MC-279395](https://bugs.mojang.com/browse/MC-279395) - Trees can generate floating over ruined portals
-   [MC-279561](https://bugs.mojang.com/browse/MC-279561) - Resin brick walls without a center post are not solid
...
-   [MC-280250](https://bugs.mojang.com/browse/MC-280250) - Projectiles sometimes desync when thrown or shot while moving
-   [MC-293615](https://bugs.mojang.com/browse/MC-293615) - Tooltip formatting of "\[item\] x \[number\]" is inconsistent across different types of items
...
-   [MC-295756](https://bugs.mojang.com/browse/MC-295756) - Pistons desync on the client, causing players to be moved differently compared to other entities
-   [MC-296006](https://bugs.mojang.com/browse/MC-296006) - Entities do not face the player correctly while in a boat
...
-   [MC-297741](https://bugs.mojang.com/browse/MC-297741) - Gravity-affected blocks in upwards bubble columns can visually clip through solid blocks above them
-   [MC-298066](https://bugs.mojang.com/browse/MC-298066) - Directly entering a bed from a mount places the player in the wrong place
-   [MC-298431](https://bugs.mojang.com/browse/MC-298431) - Boats are launched by slime blocks inconsistently
...
-   [MC-298762](https://bugs.mojang.com/browse/MC-298762) - Phantoms deflect arrows shot from bows or crossbows
...
-   [MC-299582](https://bugs.mojang.com/browse/MC-299582) - Special crafting recipes cannot be given or taken with `/recipe` and do not respect the `limited_crafting` game rule
-   [MC-300000](https://bugs.mojang.com/browse/MC-300000) - Players desync when teleporting all users riding a happy ghast
-   [MC-300831](https://bugs.mojang.com/browse/MC-300831) - `/test locate` always fails
-   [MC-301804](https://bugs.mojang.com/browse/MC-301804) - Test blocks' GUI does not pause the game in singleplayer
-   [MC-302221](https://bugs.mojang.com/browse/MC-302221) - Moving entities deflect arrows
-   [MC-302271](https://bugs.mojang.com/browse/MC-302271) - Sniffers stop sniffing after relog
-   [MC-302537](https://bugs.mojang.com/browse/MC-302537) - `/give` succeeds when trying to give more than 100 stacks of an item despite the failure message and no items being given
...
-   [MC-303493](https://bugs.mojang.com/browse/MC-303493) - The `apply_impulse` entity effect does not apply components of the impulse that would cause the affected entity to hit a solid block when used with a `tick` or `location_changed` advancement trigger
...
-   [MC-303789](https://bugs.mojang.com/browse/MC-303789) - Looking directly up with an `apply_impulse` enchantment entity effect will limit the impulse's effect on the X and Z axes to only apply on one of the two horizontal axes
...
-   [MC-304594](https://bugs.mojang.com/browse/MC-304594) - Spears can dismount interaction entities from their vehicles
-   [MC-304719](https://bugs.mojang.com/browse/MC-304719) - The `InhabitedTime` NBT tag for some chunks can be set to impossibly high values when a world is opened
...
-   [MC-305535](https://bugs.mojang.com/browse/MC-305535) - Elytra are activated after only single tapping the "Jump" key when the player is in lava
...
-   [MC-306156](https://bugs.mojang.com/browse/MC-306156) - Breezes, slimes and players between translucent blocks or fluids appear behind them with the "Improved Transparency" option enabled
...
-   [MC-306318](https://bugs.mojang.com/browse/MC-306318) - Translucent items render through the head of players with a translucent item equipped on their head with the "Improved Transparency" option enabled
...
-   [MC-306891](https://bugs.mojang.com/browse/MC-306891) - Sneaking and then opening the chat forces the IME to switch to the corresponding CJK mode
-   [MC-307122](https://bugs.mojang.com/browse/MC-307122) - The hanging roots drop from hoeing rooted dirt appears to be hardcoded
...
-   [MC-307319](https://bugs.mojang.com/browse/MC-307319) - Low performance with the Vulkan rendering backend on Intel Arc GPUs (especially on Linux)
...
-   [MC-307376](https://bugs.mojang.com/browse/MC-307376) - The CPU and GPU isn't fully utilized; the framerate is stuck at 25-35 fps
...
-   [MC-307452](https://bugs.mojang.com/browse/MC-307452) - Significant performance regression with the Vulkan rendering backend
...
-   [MC-307625](https://bugs.mojang.com/browse/MC-307625) - The glowing effect is not visible on block displays displaying End portals or End gateways
...
-   [MC-307890](https://bugs.mojang.com/browse/MC-307890) - Some textures of geyser plume particles use the wrong color in some spots
...
-   [MC-307933](https://bugs.mojang.com/browse/MC-307933) - Deepslate ores can generate on the surface by replacing tuff around sulfur springs
-   [MC-307938](https://bugs.mojang.com/browse/MC-307938) - Low performance with the Vulkan rendering backend on AMD GPUs
...
-   [MC-308008](https://bugs.mojang.com/browse/MC-308008) - The success message of `/tag` reports that it affected more entities than it did
-   [MC-308009](https://bugs.mojang.com/browse/MC-308009) - The success message of `/scoreboard players enable` reports that it affected more players than it did
-   [MC-308011](https://bugs.mojang.com/browse/MC-308011) - The success message of `/recipe` reports that it affected more players than it did
-   [MC-308012](https://bugs.mojang.com/browse/MC-308012) - The success message of `/enchant` reports that it affected more entities than it did
-   [MC-308013](https://bugs.mojang.com/browse/MC-308013) - The success message of `/effect` reports that it affected more entities than it did
-   [MC-308014](https://bugs.mojang.com/browse/MC-308014) - The success message of `/clear` reports that it affected more players than it did
-   [MC-308125](https://bugs.mojang.com/browse/MC-308125) - Scroll wheel tilting/sideways scrolling appears to move the selected hotbar slot the wrong way
-   [MC-308127](https://bugs.mojang.com/browse/MC-308127) - The "Friends Screen" key bind overrides debug hotkeys using the same key
-   [MC-308222](https://bugs.mojang.com/browse/MC-308222) - World generation heightmaps do not persist properly through chunk saving and loading
-   [MC-308254](https://bugs.mojang.com/browse/MC-308254) - Dying to a sulfur cube with a magma block doesn't increment the player's `killed_by:sulfur_cube` statistic
-   [MC-308314](https://bugs.mojang.com/browse/MC-308314) - Custom player heads do not render their texture properly when inside a sulfur cube
...
-   [MC-308405](https://bugs.mojang.com/browse/MC-308405) - `/unpublish` succeeds when the game is not published despite the failure message
...
-   [MC-308490](https://bugs.mojang.com/browse/MC-308490) - The "regular" sulfur cube archetype produces stone sounds, despite no longer using stone blocks
...
-   [MC-308752](https://bugs.mojang.com/browse/MC-308752) - Frame presentation stutters heavily with the OpenGL rendering backend on Intel Arc if a GUI is opened in borderless fullscreen mode
-   [MC-308765](https://bugs.mojang.com/browse/MC-308765) - Ender pearls don't cancel the player's velocity
-   [MC-308789](https://bugs.mojang.com/browse/MC-308789) - Pointed dripstone and sulfur spikes can be destroyed by tridents in Adventure mode
...
-   [MC-308809](https://bugs.mojang.com/browse/MC-308809) - Full-width spaces take up more space on signs now
-   [MC-308830](https://bugs.mojang.com/browse/MC-308830) - Placing beds does not send block updates to the foot half's neighboring blocks
-   [MC-308836](https://bugs.mojang.com/browse/MC-308836) - Obfuscated text no longer keeps a consistent width
-   [MC-308849](https://bugs.mojang.com/browse/MC-308849) - Creakings can take knockback
...
-   [MC-308866](https://bugs.mojang.com/browse/MC-308866) - Unable to change online settings when in a world
-   [MC-308914](https://bugs.mojang.com/browse/MC-308914) - End portals or End gateways held by glowing endermen do not glow
-   [MC-308915](https://bugs.mojang.com/browse/MC-308915) - End portals and End gateways do not glow when inside glowing minecarts
-   [MC-308916](https://bugs.mojang.com/browse/MC-308916) - Glowing TNT that renders as an End portal or End gateway does not glow
-   [MC-308917](https://bugs.mojang.com/browse/MC-308917) - Items with the `end_cube` special model type do not show a glowing outline where expected
...
-   [MC-309891](https://bugs.mojang.com/browse/MC-309891) - The `chat_restriction.disabled_by_launcher` string lacks a period, unlike similar strings
-   [MC-309893](https://bugs.mojang.com/browse/MC-309893) - The `gamerule.minecraft.fire_spread_radius_around_player.description` string lacks a period, unlike similar strings
-   [MC-309897](https://bugs.mojang.com/browse/MC-309897) - The `gui.report_to_server` string is improperly capitalized
...
-   [MC-309901](https://bugs.mojang.com/browse/MC-309901) - The `structure_block.position.x` string is improperly capitalized
...
-   [MC-309907](https://bugs.mojang.com/browse/MC-309907) - The word "whoosh" is misspelled as "woosh" in the `subtitles.block.bubble_column.upwards_inside` string
-   [MC-309908](https://bugs.mojang.com/browse/MC-309908) - The `subtitles.entity.copper_golem.no_item_get` string is missing an article before the word "item"
...
-   [MC-309923](https://bugs.mojang.com/browse/MC-309923) - The `gamerule.minecraft.entity_drops.description` string is improperly capitalized
...
-   [MC-309928](https://bugs.mojang.com/browse/MC-309928) - The `multiplayer.confirm_command.signature_required` string lacks a period
...
-   [MC-309938](https://bugs.mojang.com/browse/MC-309938) - The `quickplay.error.invalid_identifier` string is missing an article before the word "world"
...
-   [MC-310062](https://bugs.mojang.com/browse/MC-310062) - Armadillos stop following players holding spider eyes after relog
...
-   [MC-310232](https://bugs.mojang.com/browse/MC-310232) - Resizing the game window in the Realms players menu misplaces UI elements
...
-   [MC-310684](https://bugs.mojang.com/browse/MC-310684) - Camels stop following players holding cacti after relog
...
-   [MC-311235](https://bugs.mojang.com/browse/MC-311235) - Upgrading a world from before 26.1 can cause data from other worlds to get saved inside it
-   [MC-311245](https://bugs.mojang.com/browse/MC-311245) - Ruined portals can generate replacing End portal frames
-   [MC-311271](https://bugs.mojang.com/browse/MC-311271) - Some loot context parameters are missing from the `generic` loot context type
-   [MC-311272](https://bugs.mojang.com/browse/MC-311272) - The game crashes when referenced loot context parameters are not provided by certain loot tables
-   [MC-311323](https://bugs.mojang.com/browse/MC-311323) - Locating a structure creates an empty region file for every candidate chunk in ungenerated terrain
...
-   [MC-311579](https://bugs.mojang.com/browse/MC-311579) - The player can see through blocks when inside snow layers
-   [MC-311593](https://bugs.mojang.com/browse/MC-311593) - Spears' charging animation jitters from another player's perspective when swapping to a spear and charging it in the same tick
...
![An abandoned camp with a collapsed tent, unlit campfire and some green cushions.](https://launchercontent.mojang.com/v2/images/camp1280x720.jpg)